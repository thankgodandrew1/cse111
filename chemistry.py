def main():
    """This main function takes no parameter and returns nothing but it does
    call the make_periodic_table function and also prints the name and atomic mass
    """ 
    print("""
             ....Welcome....
            .....To The......
    ...
    symbol_quantity_list = parse_formula(formula, elements)

    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    molar_mass = compute_molar_mass(symbol_quantity_list, elements)

    #Calls the known_molecules function
    known_molecules = make_known_molecules_dict()

    #Call the get_formula_name function 
    name = get_formula_name(formula, known_molecules)

    # Compute the number of moles in the sample.
    number_of_moles = mass / molar_mass

    # Print the molar mass.
    print(f'\nThe molar mass of {formula}: {molar_mass:.5f} grams/mole')

    # Print the number of moles.
    print(f'The number of moles in the sample: {number_of_moles:.5f} moles')
    
    #prints the compound name
    print(f'The compound name is: {name}')
    # prints the name and atomic_mass
    # for element in elements:
    #     print(f'{element[1]} {element[2]}')

def make_periodic_table():
    """This named function stores the dict of the periodic table and posseses zero parameter
    """    
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    """
    molar_mass = 0
# first obtain the atomic weights from the elements, count the number of atoms and multiply by the atomic  mass, add all the atomic nasses
    
    # For each list in the compound symbol_quantity_list:
    # Separate the list into symbol and quantit
    for symbol, quantity in symbol_quantity_list:
        
        # Get the atomic mass for the symbol from the dictionary.
        atomic_mass_value = periodic_table_dict[symbol]

        #Retrieves the atomic mass from the symbol_key in the perioodic_table dict
        atomic_mass = atomic_mass_value[ATOMIC_MASS_INDEX]
        
        #Multiply the atomic mass by the quantity
        product = atomic_mass * quantity

        #Add the product into the molar mass
        molar_mass += product
